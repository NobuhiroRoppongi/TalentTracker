...
import json
from utils import load_skills_from_excel, generate_employees_json_from_csv
...
def load_data():
    try:
        ...
        generate_employees_json_from_csv()
        ...
        if skills is None:
            logging.warning(
                "Failed to load skills from Excel, falling back to JSON")
            try:
                with open('static/data/skills.json', 'r',
                          encoding='utf-8') as f:
                    skills = json.load(f)
...
def get_employee(employee_id):
    employees, _, _ = load_data()
    employee = next((e for e in employees if str(e.get('id')) == employee_id),
                    None)
